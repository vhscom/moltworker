...
function validateRequiredEnv(env: MoltbotEnv): string[] {
  ...
  // Check for AI Gateway, direct Anthropic, or Venice configuration
  ...
  } else if (!env.ANTHROPIC_API_KEY && !env.VENICE_API_KEY) {
    // Direct provider access requires an API key
    missing.push('ANTHROPIC_API_KEY, VENICE_API_KEY, or AI_GATEWAY_API_KEY');
  }
