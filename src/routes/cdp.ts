	...
	switch (command) {
		...
		case "printToPDF": {
			...
			if (params.printBackground)
				options.printBackground = params.printBackground as boolean;
			...
			if (params.marginTop)
				options.margin = { ...options.margin, top: `${params.marginTop}in` };
			if (params.marginBottom)
				options.margin = {
					...options.margin,
					bottom: `${params.marginBottom}in`,
				};
			if (params.marginLeft)
				options.margin = { ...options.margin, left: `${params.marginLeft}in` };
			if (params.marginRight)
				options.margin = {
					...options.margin,
					right: `${params.marginRight}in`,
				};
			if (params.pageRanges) options.pageRanges = params.pageRanges as string;
			if (params.headerTemplate)
				options.headerTemplate = params.headerTemplate as string;
			if (params.footerTemplate)
				options.footerTemplate = params.footerTemplate as string;
			if (params.preferCSSPageSize)
				options.preferCSSPageSize = params.preferCSSPageSize as boolean;
	...
	switch (command) {
		case "dispatchMouseEvent": {
			...
			switch (type) {
				...
				case "mouseWheel":
					await mouse.wheel({
						deltaX: params.deltaX as number,
						deltaY: params.deltaY as number,
					});
					break;
				default:
	...
	switch (command) {
		...
		case "setTimezoneOverride": {
			...
			await page.evaluateOnNewDocument((tz: string) => {
				...
				const _originalToString = Date.prototype.toString;
			...
			page.on("request", async (request) => {
				...
				if (patterns) {
					for (const pattern of patterns) {
						if (
							!pattern.urlPattern ||
							request.url().match(pattern.urlPattern)
						) {
